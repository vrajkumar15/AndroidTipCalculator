...
public class ExpandingCells extends Activity {
   ...
   // public ArrayList<Bytes> Byte = new ArrayList<Bytes>();
	...
	public String Raghu_Image;
	 ...
	 ExpandableListItem[] values = new ExpandableListItem[4] ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ( firstRun )
        {
                Intent i = new Intent(ExpandingCells.this, GetUserOpt.class);
                startActivity(i);
        }
       ...
       // Toast.makeText(this, Byte.size(), Toast.LENGTH_LONG).show();
        ...
        values[0] = new ExpandableListItem("Color", R.drawable.mb_color, CELL_DEFAULT_HEIGHT,
       		NewsList.get(0).getBody());
        values[1] = new ExpandableListItem("Raghukalam", R.drawable.mb_raghukalam, CELL_DEFAULT_HEIGHT,
           		NewsList.get(1).getBody());
        
//        values = new ExpandableListItem[] {
...
//        };
       ...
       // List<ExpandableListItem> mData = new ArrayList<ExpandableListItem>();
        ...
        for (int i = 0; i < (NUM_OF_CELLS-2); i++) {
            ExpandableListItem obj = values[i % values.length];
            String debug_value = String.valueOf(i % values.length);
            Log.d("DEBUG", debug_value);
		...
		client.get("http://pipes.yahoo.com/pipes/pipe.run?_id=_omfgXdL3BGGadhGdrq02Q&_render=json&sign=Virgo&url=http%3A%2F%2Fwww.astrology.com%2Fhoroscopes%2Fdaily-horoscope.rss", new JsonHttpResponseHandler() {
			...
			public void onSuccess(JSONObject response) {	
				...
				try {				
                    ...
                    Log.d("DEBUG",Horo);
                    Log.d("DEBUG", "Horo");
                    NewsList.get(2).setNews(Horo_Image, "Horoscope", Horo);
                    //adapter.notifyDataSetChanged();
                    Log.d("DEBUG", "Horo1");
                    
                    values[2] = new ExpandableListItem("Horoscope", R.drawable.mb_horoscope, CELL_DEFAULT_HEIGHT,
                            		NewsList.get(2).getBody());
                    ExpandableListItem obj = values[2];
                   // String debug_value = String.valueOf(i % values.length);
                   // Log.d("DEBUG", debug_value);
                    mData.add(new ExpandableListItem(obj.getTitle(), obj.getImgResource(),obj.getCollapsedHeight(), obj.getText()));
    ...
    public void getRaghukalam(){
		...
		NewsList.get(1).setNews(Raghu_Image, "Raghukalam", Raghukalam);
			...
			client_w.get("http://api.openweathermap.org/data/2.5/weather?lat=" + location.getLatitude() + "&lon=" + location.getLongitude() + "&APPID=055a946d7193b12bb5ad491595be1124", new JsonHttpResponseHandler() {
				...
				public void onSuccess(JSONObject response) {
					try{
						...
						NewsList.get(3).setNews("Weather", "Weather", weatherResult +" "+ cloudResult+ " " + cdescResult);
		                   values[3] = new ExpandableListItem("Weather", R.drawable.mb_weather, CELL_DEFAULT_HEIGHT,
                           		NewsList.get(3).getBody());
	                    ExpandableListItem obj = values[3];
	                     mData.add(new ExpandableListItem(obj.getTitle(), obj.getImgResource(),obj.getCollapsedHeight(), obj.getText()));
